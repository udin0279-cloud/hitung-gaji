            ...
            {items.map((x) => (
              <tr key={x.id} data-testid={`admin-leave-row-${x.id}`} className="border-b border-zinc-100 hover:bg-zinc-50/80 align-top">
                ...
                <td className="px-4 py-3">
                  ...
                  {x.time_minutes ? <div className="text-[11px] text-zinc-500 font-mono">{x.time_minutes} menit</div> : null}
                </td>
...
function StatCard({ label, value, icon: Icon, color, testId }) {
  return (
    <div className="bg-white p-4" data-testid={testId}>
      ...
      <div className="font-mono text-2xl font-semibold mt-2" style={{ color }}>{value}</div>
    </div>
  );
}
