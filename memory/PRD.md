...
- **Buku Kas — Cleanup Kasbon Filter Permanen + STRICT Normalize (2026-08-03)**: Filter permanen kasbon di tab Buku Kas sekarang **STRICT WHITELIST** — hanya `status === 'PENDING'` (dgn tolerance backward-compat "open"/"pending" lowercase). Backend `GET /cashbook/kasbon` sekarang **normalize** status output ke UPPERCASE canonical: (`open`/`pending`/empty → `PENDING`; `settled`/`paid`/`lunas`/`closed`/`done` → `PAID`). Filter status query juga menerima varian input (`open|PENDING|pending|Pending|""|null` → whitelist PENDING). Endpoint kasbon_settle/reopen/update juga normalize output. Applied di 3 layer: DB $in filter → post-normalize → client STRICT. E2E tested dengan 7 kasbon (berbagai status: `open`/`settled`/`PAID`/`LUNAS`/`PENDING`/`paid` lowercase): filter `?status=open` hanya return 3 PENDING records, semua PAID/LUNAS DITOLAK. Judul tetap "· KASBON SEMENTARA (BELUM LUNAS)".
