...
- **Buku Kas — Cleanup Kasbon Filter Permanen + STRICT Normalize + Migration (2026-08-03)**: Filter permanen kasbon di tab Buku Kas sekarang **STRICT WHITELIST** — hanya `status === 'PENDING'`. Backend `GET /cashbook/kasbon` **normalize** status output ke UPPERCASE canonical (`open`/`pending`/empty → `PENDING`; `settled`/`paid`/`lunas`/`closed`/`done` → `PAID`). Applied di 3 layer: DB $in filter → post-normalize → client STRICT. **Migration script `POST /cashbook/kasbon/migrate-status?apply=true`**: normalize permanen status di DB — dry-run mode default, timestamp `status_migrated_at` untuk audit, idempotent (2nd run = 0 changes). E2E tested dengan 7 varian records: `open`/`settled`/`PAID`/`LUNAS`/`paid`/`None`/`PENDING` → semua ter-normalize ke PENDING(3)/PAID(4). Judul tetap "· KASBON SEMENTARA (BELUM LUNAS)".
